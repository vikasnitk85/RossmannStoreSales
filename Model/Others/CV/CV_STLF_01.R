...
validation <- foreach(Store=unique(test$Store), .combine=rbind, .packages="forecast") %dopar% {
  ...
  stlfFit <- stlf(x = x, h = nrow(valData), s.window = 1, method = 'ets', ic = 'bic', opt.crit = 'mae')
...
# Save Outputs
write.csv(store_accuracy, paste0("cv/CV_STLF_", subversion, "_Store.csv"), row.names=FALSE)
write.csv(validation, paste0("cv/CV_STLF_", subversion, ".csv"), row.names=FALSE)
endTime <- Sys.time()

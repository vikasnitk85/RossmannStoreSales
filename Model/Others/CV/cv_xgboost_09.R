...
xgval <- xgb.DMatrix(data=data.matrix(train[hold, feature.names]), label=train$Sales[hold])
xgtrain <- xgb.DMatrix(data=data.matrix(train[-hold, feature.names]), label=train$Sales[-hold])

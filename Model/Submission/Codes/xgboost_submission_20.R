...
train <- train[, setdiff(names(train), c("Date", "StateHoliday", "Customers"))]
...
h <- which(train$year >=2015 & train$month >= 7)
...
#-------------------------------------------------------------------------------
valData <- train[h, ]
predVal <- exp(predict(Model, dval, ntreelimit=1790)) - 1
valData$Preds <- predVal
valData <- valData[valData$Store==1, ]

RMSPE <- function(preds, lab) {
  err <- sqrt(mean((preds/lab-1)^2))
  return(list(metric = "RMSPE", value = err))
}
RMSPE(valData$Preds, valData$Sales)


pred1 <- exp(predict(Model, data.matrix(test[, feature.names]), ntreelimit=1790)) - 1
summary(pred1)

...
train$year_diff <- train$year - train$CompetitionOpenSinceYear
test$year_diff <- test$year - test$CompetitionOpenSinceYear

train$month_diff <- (train$year - train$CompetitionOpenSinceYear) * 12 + (train$month - train$CompetitionOpenSinceMonth)
test$month_diff <- (test$year - test$CompetitionOpenSinceYear) * 12 + (test$month - test$CompetitionOpenSinceMonth)
...
pred1 <- exp(predict(Model, xgtest, ntreelimit=887)) - 1
summary(pred1)

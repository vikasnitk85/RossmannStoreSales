...
feature.names <- setdiff(names(trainData), c("Date", "Sales", "Customers", "Open"))
trainData$Preds <- NA
valData$Preds <- NA
testData$Preds <- NA

storeAccuracy <- data.frame(matrix(NA, nrow=length(unique(testData$Store)), ncol=3))
names(storeAccuracy) <- c("Store", "valRMSPE", "trainRMSPE")
...
Count <- 0
for(Store in unique(valData$Store)) {
    Count <- Count + 1
    # Store accuracy
    storeAccuracy[Count, 1] <- Store
    storeAccuracy[Count, 2] <- RMSPE1(trainData[trainData$Store==Store, "Preds"], trainData[trainData$Store==Store, "Sales"])
    storeAccuracy[Count, 3] <- RMSPE1(valData[valData$Store==Store, "Preds"], valData[valData$Store==Store, "Sales"])
}
write.csv(storeAccuracy, paste0("Model/Submission/xgboost_Performance_", subversion, ".csv"), row.names=FALSE)

valData1 <- valData[valData$Store %in% testData$Store, c("Sales", "Preds")]
valData1 <- valData1[complete.cases(valData1), ]
valRMSPE <- RMSPE1(valData1[, "Preds"], valData1[, "Sales"])
valRMSPE

summary(storeAccuracy[storeAccuracy$Store %in% testData$Store, ])
mean(storeAccuracy[storeAccuracy$Store %in% testData$Store, 2])

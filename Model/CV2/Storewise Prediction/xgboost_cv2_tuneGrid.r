...
tuneGrid <- expand.grid(gamma=c(0), max_depth=3:5, min_child_weight=c(1, 5, 10),
                        subsample=seq(0.4, 0.7, 0.1), colsample_bytree=1, lambda=0:4, alpha=0:4)
tuneGrid$trainRMSPE <- NA
tuneGrid$valRMSPE <- NA
tuneGrid$testRMSPE <- NA
tuneGrid$Time <- NA

for(i in 36:nrow(tuneGrid)) {
  startTime <- Sys.time()
  trainData$Preds <- NA
  valData$Preds <- NA
  testData$Preds <- NA
  ...
  write.table(tuneGrid, "Model/CV2/xgboost_cv2_tuneGrid.txt", row.names=FALSE)
  print(tuneGrid[i, ])
  rm(list=setdiff(ls(all=TRUE), c("feature.names", "getHistory", "RMSPE", "RMSPE1", "storeToKeep", "testData", "trainData", "tuneGrid", "valData")))
  gc()
}

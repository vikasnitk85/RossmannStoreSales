...
for(Store in storeToKeep) {
    ...
    write.csv(storeAccuracy, paste0("Model/CV2/xgboost_cv2_", subversion, ".csv"), row.names=FALSE)
    rm(list=setdiff(ls(all=TRUE), c("Count", "feature.names", "RMSPE", "RMSPE1", "startTime", "storeAccuracy", "storeToKeep",
        "subversion", "testData", "trainData", "valData", "Store")))
    gc()

...
Count <- 0
Store <- 1
tuneGrid <- expand.grid(max_depth=3:6, subsample=0.4, alpha=0:4, lambda=0:4)
...
for(i in 1:nrow(tuneGrid)) {
    ...
    # Store accuracy
    tuneGrid$trainRMSPE[i] <- RMSPE1(trainData[trainData$Store==Store, "Preds"], trainData[trainData$Store==Store, "Sales"])
    tuneGrid$valRMSPE[i] <- RMSPE1(valData[valData$Store==Store, "Preds"], valData[valData$Store==Store, "Sales"])
    tuneGrid$testRMSPE[i] <- RMSPE1(testData[testData$Store==Store, "Preds"], testData[testData$Store==Store, "Sales"])
    print(tuneGrid[i, ])
}

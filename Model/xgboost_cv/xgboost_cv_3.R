...
# Cross Validation
set.seed(1024)
cv_result <- xgb.cv(data = xgtrain, nfold = 10, nrounds = nrounds,
...
bst <- xgboost(data = xgtrain, nrounds = nrounds, 
               objective = "reg:linear", feval = RMSPE, eta = eta,
			   gamma = 0, lambda = 1, alpha = 0, nthread = 16,
			   max_depth = 6, min_child_weight = 1, verbose = 1,
			   subsample = 1, colsample_bytree = 1, eval_metric=RMSPE)
...
for(i in 1:nrow(cv_result)) {
  ...
  cv_result[i, testing:=test_err]
  print(i)
}
head(cv_result)

cv_result[order(testing), ]
endTime <- Sys.time()
...
# Save results
#-------------------------------------------------------------------------------
save(startTime, endTime, timeTaken, nrounds, eta, cv_result, test_err, file="Model/xgboost_cv/xgboost_cv_3.RData")

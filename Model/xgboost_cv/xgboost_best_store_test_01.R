...
train <- train[order(Store, Date), ]
trainingData <- data.frame(train[, .SD[1:(length(Date)-41)], by="Store"])
...
for(Store in unique(trainingData$Store)) {
  ...
  Accuracy <- Accuracy[order(Accuracy$test_RMSPE), ]
  summary(Accuracy)
  head(Accuracy)
  ...
  set.seed(1024)
  tempOut <- capture.output(
               bst <- xgb.train(data = xgtrain, nrounds = Accuracy[1, 2],
               feval = RMSPE, params = param0,
               watchlist = watchlist, nthread = 16)
             )
  ...
  print(unlist(c(Store, Accuracy[1, ], timeTaken)))
}
...
# Save results
#-------------------------------------------------------------------------------
save(testingData, error, file=paste0("Model/xgboost_cv/xgboost_best_store_test_", subversion, ".RData"))

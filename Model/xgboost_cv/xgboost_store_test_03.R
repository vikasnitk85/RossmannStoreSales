...
for(Store in unique(trainingData$Store)) {
  ...
  for(i in 1:length(max_depths)) {
	...
	set.seed(1024)
	CV_History <- xgb.cv(params = param0, data = xgtrain1, nrounds = nrounds, nfold = 10, feval = RMSPE, nthread = 16, verbose = F)
    CV_History[, tree:=1:nrounds]
	CV_History[, max_depth:=max_depths[i]]
	CV_History <- CV_History[order(test.RMSPE.mean), ]
	CV_History[1, tree]
	
    watchlist <- list('train' = xgtrain1, 'val' = xgtest)
    set.seed(1024)
    tempOut <- capture.output(
                 bst <- xgb.train(data = xgtrain1, nrounds = nrounds,
    ...
    # head(cv_result)
	cv_result[cv_result$tree==CV_History[1, tree], ]
  ...
  testingData$Pred[testingData$Store==Store] <- predict(bst, newdata = xgtest, ntreelimit = 2000)

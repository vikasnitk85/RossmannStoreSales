...
for(Count in 1:length(max_depths)) {
  # Other parameters
  startTime <- Sys.time()
  print(Count)
  print(startTime)
  ...
  # Save results
  #-------------------------------------------------------------------------------
  save(startTime, endTime, timeTaken, nrounds, cv_result, file=paste0("Model/xgboost_cv/xgboost_cv_", subversions[Count], ".RData"))
}
